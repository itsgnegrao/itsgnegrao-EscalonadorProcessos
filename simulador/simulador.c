...
//Relógio do sistema
uint64_t relogio;
//Ponteiro estrutura de saida.
arq_saida_t* saida = NULL;
...
int main(int argc, char** argv) {
    ...
    *(strstr(experimento->arq_processos, "\n")) = '\0';
    ...
    saida = CRIA_arq_saida(processos);
    ...
    while (!LISTA_BCP_vazia(prontos)
        ...
        if (novos->data[0]->entrada == relogio) {
            //ADICIONA EVENTO
            CRIA_ADC_evento(saida, "criacao", relogio, novos->data[0]);
        ...
        if (executando) {
            ...
            if (executando->eventos[executando->proxEvento]->tempo == executando->tempoExecutado) {
                ...
                if (executando->eventos[executando->proxEvento]->evento == EVT_TERMINO) {
                    ...
                    //ADICIONA EVENTO
                    CRIA_ADC_evento(saida, "termino", relogio, executando);
                    ...
                    //ADICIONA EVENTO
                    CRIA_ADC_evento(saida, "bloqueio", relogio, executando);
        ...
        //verifica se tem algum processo executando e o timeslice==1 quer dizer que estourou seu tempo de execução;
        if (executando != NULL && executando->timeSlice == 1) {
            //ADICIONA EVENTO
            CRIA_ADC_evento(saida, "quantum ex", relogio, executando);
        }
        ...
        for (i = 0; i < bloqueados->tam; i++) {
            ...
            if (bloqueados->data[i]->tempoBloqueio <= 0) {
                //ADICIONA EVENTO
                CRIA_ADC_evento(saida, "desbloqueio", relogio, bloqueados->data[i]);
                bcp_t* p;
                p = bloqueados->data[i];
        ...
        if (!executando) {
            ...
            if (executando != NULL) {
                trocas_de_contexto++;
                //ADICIONA EVENTO
                CRIA_ADC_evento(saida, "execucao", relogio, executando);
    ...
    printf("Tempo médio de espera: %d\n", saida->termino_n/*saida->TME*/);
    GRAVAR_saida(saida, experimento->arq_saida, processos->nProcessos);
